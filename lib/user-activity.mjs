...
//     pasted or machine-injected prompts (which aren't typed at all), so the
//     estimate is capped at TYPE_CAP_MS — a long prompt never reads as an hour
//     of typing.
...
const TYPE_CAP_MS = 4 * 60_000;          // typing never shown longer than this —
                                         // a long prompt is almost always pasted or
                                         // machine-injected, not typed at 40 wpm
...
// ── estimators ─────────────────────────────────────────────────────────────
const typingMs = (text) => Math.min(TYPE_CAP_MS, Math.round((text ? text.length : 0) * TYPE_MS_PER_CHAR));
