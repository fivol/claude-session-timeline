...
export function parseTranscript(filePath) {
  ...
  const toolSpans = [];
  const modelCounts = new Map(); // model id -> assistant-message count
  ...
  for (const line of raw.split('\n')) {
    ...
    if (d.type === 'user' && t != null) {
      ...
      // Track which AI model produced this turn. '<synthetic>' tags injected
      // (non-model) messages, so it's excluded from the "main model" tally.
      const mdl = (d.message || {}).model;
      if (mdl && mdl !== '<synthetic>') modelCounts.set(mdl, (modelCounts.get(mdl) || 0) + 1);
      let textChars = 0;
      for (const b of content) {
  ...
  // The session's "main" model is the one that produced the most turns.
  let model = null, modelBest = 0;
  for (const [k, v] of modelCounts) if (v > modelBest) { modelBest = v; model = k; }
